...
assert.match(js, /initializeTenantContext\(\)/);
assert.match(js, /tenantContext\.status !== TENANT_CONTEXT_STATES\.READY/);
assert.match(js, /doc\(db, "barbearias", tenantContext\.tenantId, "configuracoes", "identidade"\)/);
assert.doesNotMatch(js, /BARBEARIA_ATUAL_ID/);
assert.match(js, /await waitForAdminAccessGuard\(\)/);
