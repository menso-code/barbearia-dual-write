...
function traduzErro(code, errorName = "") {
  const mapa = {
    ...
    "auth/user-disabled": "Esta conta de teste está desativada.",
    "auth/operation-not-allowed": "O login por e-mail e senha não está habilitado na homologação.",
    "auth/network-request-failed": "Não foi possível acessar a autenticação. Verifique sua conexão e tente novamente.",
  ...
  // Diagnóstico exclusivo da homologação: mostra apenas o identificador
  // técnico seguro do Firebase. Nunca expõe senha, token, mensagem interna
  // ou dados do usuário.
  const codigoSeguro = typeof code === "string" && /^(?:auth|app|functions|firestore|storage)\/[a-z0-9._-]+$/i.test(code)
    ? code
    : "desconhecido";
  const categoriaSegura = ["Error", "TypeError", "FirebaseError"].includes(errorName)
    ? errorName
    : "desconhecida";
  return `Não foi possível entrar na homologação (código: ${codigoSeguro}; tipo: ${categoriaSegura}).`;
}
...
formCadastro?.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    sessionStorage.removeItem("cadastroEmAndamento");
    showMsg(msgAuth, traduzErro(err?.code, err?.name));
    btn.disabled = false;
  }
});
...
formLogin?.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // Somente para diagnóstico em homologação: não registra e-mail ou senha.
    console.warn("Falha de login na homologação.", {
      code: err?.code || "sem-codigo",
      name: err?.name || "sem-nome",
      type: typeof err,
      hasMessage: Boolean(err?.message),
    });
    showMsg(msgAuth, traduzErro(err?.code, err?.name));
    btn.disabled = false;
  }
