...
const records = buildCustomerRecords({
  now,
  clients: [
    { id: "alice", data: { nome: "Álice Silva", telefone: "(11) 99999-0000" } },
...
assert.equal(records.length, 3, "clientes sem agendamento também devem aparecer");
...
assert.equal(filterCustomerRecords(records, { search: "999990000" }).length, 1);
