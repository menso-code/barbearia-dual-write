...
test("Slice 6 migra somente as duas transições aprovadas e mantém 32 comandos", () => {
  ...
  assert.equal(allCommands.filter((command) => !DYNAMIC_TENANT_COMMANDS.includes(command)).length, 18);
...
test("OTHER_18_COMMANDS_FAIL_CLOSED_FOR_NEW_TENANTS", () => {
  ...
  assert.equal(remaining.includes("admin.assinatura.recusar"), true);
  assert.equal(remaining.includes("bloqueio.criar"), true);
});
