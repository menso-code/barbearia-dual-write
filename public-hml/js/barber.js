...
async function assinarAgenda() {
  ...
  cancelarAgendaListener = onSnapshot(agendaQuery, (snap) => { agendamentos = snap.docs.map((item) => ({ id:item.id, ...item.data() })); renderizarAgenda(); }, (erro) => { console.error("Falha ao atualizar agenda do barbeiro.", erro); mostrarErroAgenda(); });
  cancelarBloqueioListener = onSnapshot(bloqueioQuery, (snap) => { bloqueios = snap.docs.map((item) => ({ id:item.id, ...item.data() })); renderizarAgenda(); }, (erro) => { console.error("Falha ao atualizar agenda do barbeiro.", erro); mostrarErroAgenda(); });
}
  ...
  try {
    ...
    if (remarcacaoAtual) {
      const dadosReagendamento = {
        servico_id: comando.servico_id,
        data: comando.data,
        horario: comando.horario,
        cliente_nome: comando.cliente_nome,
        cliente_whatsapp: comando.cliente_whatsapp,
      };
      await reagendarAgendamento(db, remarcacaoAtual, dadosReagendamento);
...
async function atualizarStatus(item, status) {
  ...
  await executarComandoOperacional(`agenda.${status}`, { appointmentId: item.id });
}
