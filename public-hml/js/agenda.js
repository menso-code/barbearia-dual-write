...
let funcionamentoAtual = null;
export function definirFuncionamento(config) { funcionamentoAtual = config || null; }
...
export async function obterFechamentoGlobal(db, data) {
  ...
  const [configSnap, fechamentoSnap, aberturaSnap] = await Promise.all([getDoc(doc(db, "configuracoes", CONFIG_FUNCIONAMENTO)), getDoc(doc(db, "fechamentos_globais", data)), getDoc(doc(db, "fechamentos_globais", `abertura_${data}`))]);
  definirFuncionamento(configSnap.exists() ? configSnap.data() : null);
  const abertura = aberturaSnap.exists() && aberturaSnap.data().ativo !== false && aberturaSnap.data().tipo === "abertura" ? aberturaSnap.data() : null;
  ...
  if (!abertura && fechamentoSemanal(configSnap.exists() ? configSnap.data() : null, data)) return { fechado: true, motivo: "Fechamento semanal" };
  return { fechado: false, motivo: "", abertura };
}
...
function periodosPadrao(data) {
  const dia = new Date(`${data}T12:00:00`).getDay();
  if (Array.isArray(funcionamentoAtual?.periodos_semana?.[dia])) return funcionamentoAtual.periodos_semana[dia];
  if (dia === 0) return [];
  return [{ inicio: "08:30", fim: "12:00" }, { inicio: "13:00", fim: dia <= 4 ? "19:30" : "20:30" }];
}
function interseccao(global, pessoal) {
  const out = [];
  for (const a of global) for (const b of pessoal) { const inicio = Math.max(paraMinutos(a.inicio), paraMinutos(b.inicio)); const fim = Math.min(paraMinutos(a.fim), paraMinutos(b.fim)); if (fim > inicio) out.push({ inicio: paraHorario(inicio), fim: paraHorario(fim) }); }
  return out;
}
export function periodosDoBarbeiro(barbeiro, data) {
  ...
  const dia = new Date(`${data}T12:00:00`).getDay();
  if (funcionamentoAtual?.dias_fechados_semana?.[dia] === true) return [];
  const global = periodosPadrao(data);
  return configuracao === false ? [] : Array.isArray(configuracao) ? interseccao(global, configuracao) : global;
}
...
export async function horariosDisponiveis(db, { barbeiro, barbeiroId, data, duracao }) {
  ...
  if (fechamento.abertura) funcionamentoAtual = { ...funcionamentoAtual, dias_fechados_semana: { ...(funcionamentoAtual?.dias_fechados_semana || {}), [new Date(`${data}T12:00:00`).getDay()]: false }, periodos_semana: { ...(funcionamentoAtual?.periodos_semana || {}), [new Date(`${data}T12:00:00`).getDay()]: [{ inicio: fechamento.abertura.inicio_horario, fim: fechamento.abertura.fim_horario }] } };
