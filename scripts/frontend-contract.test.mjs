...
assert.match(studioSettings, /initializeTenantContext\(\)/);
assert.match(studioSettings, /tenantContext\.status !== TENANT_CONTEXT_STATES\.READY/);
assert.match(studioSettings, /doc\(db, "barbearias", tenantContext\.tenantId, "configuracoes", "identidade"\)/);
assert.doesNotMatch(studioSettings, /BARBEARIA_ATUAL_ID/);
