...
function obterClienteIdOperacional() {
  return uidOperacionalAtual || usuarioAtual?.uid || "";
}
...
async function atualizarMenuPorPermissao(user, versao) {
  ...
  const userRole = document.getElementById("user-role");
  if (userRole) userRole.textContent = acesso.roles.length ? acesso.roles.join(" + ") : "Sem papel";

  const itens = [];
  if (acesso.isClient) itens.push(criarLinkPrivilegiado("app.html", "Área do cliente"));
...
async function carregarAssinaturasCliente() {
  ...
  try {
    const [planosAtivos, assinaturasSnap] = await Promise.all([
      ...
      getDocs(query(collection(db, "solicitacoes_assinatura"), where("cliente_id", "==", obterClienteIdOperacional()))),
    ]);
...
document.getElementById("btn-confirmar-solicitacao-assinatura")?.addEventListener("click", async (evento) => {
  ...
  try {
    ...
    const anteriores = await getDocs(query(collection(db, "solicitacoes_assinatura"), where("cliente_id", "==", obterClienteIdOperacional())));
    if (anteriores.docs.some((item) => item.data().plano_id === plano.id && item.data().status === "PENDENTE")) {
      throw new Error("SOLICITACAO_EXISTENTE");
    }

    const clienteSnap = await getDoc(doc(db, "clientes", obterClienteIdOperacional()));
...
document.getElementById("form-agendar").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    await criarAgendamento(db, {
      cliente_id: obterClienteIdOperacional(),
    ...
    try {
      ...
      console.warn(
        "Agendamento confirmado; não foi possível atualizar a lista agora.",
        refreshError
      );
    }
...
async function carregarMeusAgendamentos() {
  ...
  const q = query(
    ...
    where("cliente_id", "==", obterClienteIdOperacional()),
    orderBy("data", "desc")
  );
  const snap = await getDocs(q);
