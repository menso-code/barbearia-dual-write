...
test("Slice 14 registra somente aprovação tenant-scoped e mantém 32 comandos", () => {
  assert.equal(allCommands.length, 32); assert.equal(DYNAMIC_TENANT_COMMANDS.includes(command), true); assert.equal(allCommands.filter((item) => !DYNAMIC_TENANT_COMMANDS.includes(item)).length, 10);
...
test("OTHER_10_COMMANDS_FAIL_CLOSED_FOR_NEW_TENANTS e COMMAND_COUNT permanece 32", () => {
  const remaining = allCommands.filter((item) => !DYNAMIC_TENANT_COMMANDS.includes(item)); assert.equal(remaining.length, 10); assert.equal(remaining.includes(command), false); assert.equal(remaining.includes("agenda.criar"), true);
});
